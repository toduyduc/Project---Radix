...
//lấy số lượng comment chưa xử lý(status=0)
function getCountComments(){
    $sql = "SELECT id FROM comments WHERE status=0";
    $count = getRows($sql);
    return $count;
}
...
function getComment($comment_id){
    $listComments = firstRow("SELECT `name` FROM comments WHERE id=$comment_id");
    return $listComments;
}
...
function getMenu($dataMenu,$isSub=false){
    ...
    if(!empty($dataMenu)){
        echo ($isSub==false)?'<ul class="nav menu">':'<ul class="dropdown">';
